import { buildClassicMap, isWalkableTile } from '/src/game-core.mjs';
...
function worldToCanvas(x, y) {
  return {
    x: (x / map.worldWidth) * battlefield.width,
    y: (y / map.worldHeight) * battlefield.height,
  };
}
...
function eventToWorld(event) {
  ...
  return {
    x: ((event.clientX - rect.left) / rect.width) * map.worldWidth,
    y: ((event.clientY - rect.top) / rect.height) * map.worldHeight,
  };
}
...
function drawTerrain() {
  ...
  const scaleX = battlefield.width / map.worldWidth;
  const scaleY = battlefield.height / map.worldHeight;
...
function drawZones() {
  for (const zone of snapshot?.zones ?? []) {
    ...
    const radius = (zone.radius / map.worldWidth) * battlefield.width;
